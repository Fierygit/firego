            ...
            <div className='w-full min-h-screen z-10 fixed top-0 left-0 flex items-center justify-center bg-gray-900 bg-opacity-80' onClick={cancel}>
                <div className='flex items-center z-20 justify-center py-6 w-11/12 max-w-md flex-col bg-gray-100 dark:bg-gray-800 rounded-xl' onClick={e => e.stopPropagation()}>
                    ...
                    <div className='flex items-center justify-evenly w-full'>
                        <button ref={btnRef} className='disabled:opacity-50 rounded-lg w-1/5 h-7 bg-green-400 text-gray-100 dark:text-black font-bold select-none focus:outline-none' onClick={onClick}>confirm</button>
...
export function ConfirmDialog({ id, name, callback }) {
    ...
    return (
        <Fragment>
            ...
            <button className="absolute hidden group-hover:block transform right-0 w-8 md:w-10 h-4/5 hover:scale-125 focus:outline-none" onClick={() => setVisible(!visible)}>
